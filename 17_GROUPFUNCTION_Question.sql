...
--학과별 평점의 최대 평균
--학과별 평점의 평균을 가져오는 쿼리 만들어진 가상테이블의 전체 데이터를 사용해서
--통계를 내기 때문에 별도의 GROUP BY는 필요없다.
SELECT MAX(AVG(AVR))
    FROM STUDENT
    GROUP BY MAJOR;
...
--부서 중 평균급여가 가장높은 부서번호와 평균급여(EMP 테이블만 사용)
...
--WHERE절에서 그룹함수를 쓰는 방식은 그룹함수를 사용한 쿼리를 서븐쿼리로 묶어서 
--테이블로 만든다음 사용한다.
SELECT DNO
    , AVG(SAL)
    FROM EMP
...
--임용년도가 동일한 교수의 수를 조회
SELECT HI.HIRE, COUNT(HI.HIRE)
FROM (SELECT TO_CHAR(HIREDATE, 'YYYY') HIRE
      FROM PROFESSOR) HI
GROUP BY HI.HIRE;
